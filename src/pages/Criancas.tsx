...
export const Crianças: React.FC = () => {
  ...
  const loadDepartmentData = async () => {
    try {
      ...
      // Buscar ou criar departamento de Crianças
      let Crianças: Department | null = null;
      
      try {
        const departments = await DepartmentService.getAllDepartments();
        Crianças = departments.find(d => d.type === 'criancas') || null;
      } catch (err) {
        ...
        if (hasRole(['pastor'])) {
          try {
            const departmentId = await DepartmentService.createDepartment({
              name: 'Crianças',
              type: 'criancas',
              description: 'Departamento de Crianças - Responsável por recepção, porta, altar e ordem',
              leaderId: user?.id || '',
              members: [],
              color: '#4caf50',
              isActive: true,
      ...
      setDepartment(Crianças || null);

      if (Crianças) {
        // Carregar membros do departamento
        const deptMembers = await UserService.getUsersByDepartment(Crianças.id);
        setMembers(deptMembers);

        // Carregar funções
        const funcList = await DepartmentService.getDepartmentFunctions(Crianças.id);
        setFunctions(funcList);

        // Carregar escalas
        const scheduleList = await ScheduleService.getDepartmentSchedules(Crianças.id);
  ...
  const handleAddMember = async () => {
    ...
    try {
      ...
      setSuccess(`${selectedUser.name} adicionado ao Crianças!`);
  ...
  const handleCreateDepartment = async () => {
    try {
      ...
      const departmentId = await DepartmentService.createDepartment({
        name: 'Crianças',
        type: 'criancas',
        description: 'Ministério Infantil (Salinha de Crianças) - Responsável pelo ensino bíblico e cuidado com as crianças',
        leaderId: user?.id || '',
        members: [],
        color: '#4caf50',
        isActive: true,
      ...
      // Criar funções padrão para o Ministério Infantil
      const defaultFunctions = [
        { 
          name: 'Tia (Professora Principal)', 
          description: 'Responsável por ministrar a aula, conduzir o ensino bíblico e as atividades principais com as crianças.',
          isRequired: true 
        },
        { 
          name: 'Ajudante', 
          description: 'Apoiar a professora principal, ajudando a organizar as crianças, preparar materiais e dar suporte durante a ministração.',
          isRequired: false 
        },
      ];
    ...
    return (
      <Box sx={{ p: 3 }}>
        <Alert severity="warning" sx={{ mb: 2 }}>
          Departamento de Crianças não encontrado.
        </Alert>
          ...
          <Card>
            <CardContent>
              <Typography variant="h6" gutterBottom>
                Criar Departamento de Crianças
              </Typography>
  ...
  return (
    <Box sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
        <Typography variant="h4">Crianças</Typography>
      ...
      <TabPanel value={tabValue} index={0}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
          <Typography variant="h6">Membros do Crianças</Typography>
          {canEdit && (
            <Button
              variant="contained"
              startIcon={<Add />}
      ...
      <TabPanel value={tabValue} index={1}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 2 }}>
          <Typography variant="h6">Funções do Ministério Infantil</Typography>
          {canEdit && (
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    try {
                      ...
                      setSuccess('Funções padrão criadas com sucesso!');
                      await loadDepartmentData();
                    } catch (err: any) {
        ...
        <Box sx={{ mb: 2 }}>
          <Alert severity="info">
            <strong>Funções do Ministério Infantil:</strong> Tia (Professora Principal), Ajudante
          </Alert>
        </Box>
      ...
      <Dialog open={addMemberDialog} onClose={() => setAddMemberDialog(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Adicionar Membro ao Crianças</DialogTitle>
        <DialogContent>
          <Autocomplete
      ...
      <Dialog open={addFunctionDialog} onClose={() => setAddFunctionDialog(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          <TextField
            ...
            placeholder="Ex: Tia (Professora Principal), Ajudante"
          />
          <TextField
            label="Descrição"
            fullWidth
            ...
            placeholder="Ex: Responsável por ministrar a aula e conduzir o ensino bíblico"
          />
        </DialogContent>
        <DialogActions>
